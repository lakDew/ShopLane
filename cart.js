    ...
    function cartItem(){
      ...
      if(quantity!=null){
          ...
          for(let i=0;i<id.length;i++){
            ...
            total=total+(obj.price*quantity[i])
            finalAmount=total;
            ...
            p2.innerHTML=`X ${quantity[i]}`
            detail.appendChild(p2);

            let p3=document.createElement('p');
            p3.innerHTML=`Amount: Rs ${obj.price*quantity[i]}`
            detail.appendChild(p3);

            leftDiv.appendChild(product)
          ...
          let p1=document.createElement('p');
          p1.innerHTML='Total Amount';
          rightDiv.appendChild(p1);

            
          let p2=document.createElement('p');
          p2.id='amount';
          p2.innerHTML=`Amount: Rs ${total}`;
          rightDiv.appendChild(p2);

          let promo=document.createElement('p');
          promo.innerHTML='HAVE COUPON';
          rightDiv.appendChild(promo);
          

          let promoInput=document.createElement('p');
          promoInput.classList.add('promo-code','d-none')
          rightDiv.appendChild(promoInput)

          let input=document.createElement('input');
          promoInput.appendChild(input);
          let apply=document.createElement('span')
          apply.innerHTML='CANCEL';
          promoInput.appendChild(apply);

          

          let coupontext=document.createElement('p');
          coupontext.classList.add('coupon-text','d-none')
          coupontext.innerHTML='please enter valid promocode';
          rightDiv.appendChild(coupontext)

          let remove=document.createElement('p');
          remove.classList.add('remove','d-none');
          ...
          input.addEventListener('input',applyBtn)
          apply.addEventListener('click',promoApply)
   ...
   function applyBtn(e){
      ...
      coupontext.classList.add('d-none');
      coupontext.innerHTML='please enter valid promocode';
      checkRemove()
      if(e.target.value!=''){
        let input =document.querySelector('.promo-code span');
        input.innerHTML='APPLY';
   ...
   function promoApply(){
    ...
    if(input.innerHTML=='CANCEL'){
      document.querySelector('.promo-code').classList.add('d-none');
      ...
      let coupontext=document.querySelector('.coupon-text')
      coupontext.innerHTML=discount+" % discount applid";
      coupontext.classList.remove('d-none');
      checkRemove()
    }
    }
    else{
     
       if(code.slice(0,6)=='edyoda' && ((Boolean(Number(code.slice(6)))!=false))&& Number(code.slice(6))<=40){

        
       
        amount=finalAmount-((finalAmount*Number(code.slice(6)))/100);
        amountElement.innerHTML=`Amount: Rs ${amount.toFixed()}`;
        document.querySelector('.promo-code').classList.add('d-none');
        let input =document.querySelector('.promo-code input');
        input.value='';
        let coupontext=document.querySelector('.coupon-text');
        coupontext.innerHTML=code.slice(6)+" % discount applid";
        coupontext.classList.remove('d-none');
        discount=Number(code.slice(6));
        code='';
        let btn =document.querySelector('.promo-code span');
   ...
   function  removePromo(){
    ...
    let check=document.querySelector('.coupon-text');
    check.classList.add('d-none');
    discount=0;
    amountElement.innerHTML=`Amount: Rs ${finalAmount.toFixed()}`;
    let coupontext=document.querySelector('.coupon-text')
    coupontext.classList.add('d-none');
    coupontext.innerHTML='please enter valid promocode';
   ...
   function checkRemove(){
    ...
    if(check.innerHTML=='please enter valid promocode'){
       remove.classList.add('d-none')
    }
    else{
